<template>
  <div class="job-info">
    <i class="el-icon-printer" @click="$router.push('/print/' + userId+ '?type=job')" />
    <!-- 基础信息 -->
...
import { getEmployeeJob, saveEmployeeJob } from '@/api/employees'
export default {
  data() {
    return {
  ...
  methods: {
    async  getEmployeeJob() {
      this.formData = await getEmployeeJob(this.userId)
    },
    async saveEmployeeInfo() {
      await saveEmployeeJob(this.formData)
